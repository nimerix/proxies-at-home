...
export default function CardCellLazy({ card, state, hasImage, ensureProcessed, children }: Props) {
  const { ref, visible } = useOnScreen<HTMLDivElement>('400px');

  useEffect(() => { if (visible) void ensureProcessed(card); }, [visible, card, ensureProcessed]);
  ...
  return (
    <div ref={ref} className="relative">
      ...
      <div onClick={() => { if (state === 'error') void ensureProcessed(card); }}>
        {children}
      </div>
    </div>
  );
